#coding:utf8
__time__ = '2018/5/10 15:32'
__author__ = 'SYJ'


# -*- coding: utf-8 -*-
#导入各种库
import requests,arrow,yaml
...
@ddt.ddt
class Uenpayrms(unittest.TestCase):
    '''用户提现接口'''
    pass
    # global data
    ...
    def setUpClass(cls):
        ...
        '''获得类名和当前函数名'''
        cls.classname=Uenpayrms().__class__.__name__
        cls.currentfun=Uenpayrms().get_current_function_name()
        ...
        cls.log.info('执行开始%s---------------------------------------'%format(Uenpayrms().get_current_function_name()))

        '''登录系统保持session'''
        requests.packages.urllib3.disable_warnings()
        url=cls.read['rms']['url']
        s = requests.session()
        cls.login=uenpay(s)
        cls.login.login(url)
    ...
    def tearDownClass(cls):
        '''关闭数据库'''
        # cls.conn.close()
        cls.log.info('执行结束%s---------------------------------------' % format(Uenpayrms().get_current_function_name()))
    ...
    def test_3uenpay(self):
        '''用户管理系统'''
        # self.log.info('执行开始%s---------------------------------------' % format(Uenpayrms().get_current_function_name()))
        ...
        url='https://test.uenpay.com/uenrms/common/menu/admin'
        ...
        self.assertEqual('风险管理',req[0]['children'][0]['text'],'gg思密达')
        # result=self.Oracle.orc_select(sql)    #调用数据库函数查询数据库数据进行断言
        # print (result)


    def test_4RmsAgentInformation(self):
        '''查询商户基本信息'''
        url='https://test.uenpay.com/uenrms/shopInfo/selectShops?time=1525940358855'
        # self.log.info('执行开始%s---------------------------------------' % format(Uenpayrms().get_current_function_name()))
        body = {'page': '1',
              'rows':'10',
              'shopName':'',
        ...
        header = self.read['header']['header']
        # print (header)
        req=self.login.Uen(fangshi='POST',url=url,param=body,headers=header)
        print (req)
        result=self.Oracle.orc_select(sql)    #调用数据库函数查询数据库数据进行断言
        print (req['rows'][9]['orgName'])
        self.assertIn('分润',req['rows'][9]['orgName'],'GG思密达')
