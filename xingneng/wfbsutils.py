...
class Uenoracle():
    ...
    def orc_select(self,sql):
        '''封装查询语句'''
        # cursor=db.cursor()
        db=self.orc_connect(self,'ORACLE')  #调用同类中的方法
        cursor=db.cursor(cx_Oracle)
        cursor.execute(sql)
    ...
    def read_valid_code(self):
        ...
        message = self.orc_select(sql1)[0][
            'LAST_VALIDATOR_CODE']
        return message
...
if __name__ == '__main__':
    # sql1 = "select * from AGENT_SEND_MSG_LOG order by CREATE_DATE desc "
    a=Uenoracle(project='DailiApp',configname='app_config',confignYaml='config.yaml')
    # print (a)
    sql = "select * from core_Device where DEV_NO='C020000000000001'"
    orgid=a.orc_select(sql)
    print (orgid)
