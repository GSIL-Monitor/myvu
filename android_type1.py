...
class PaquPhone():
    ...
    def search_phone_type(self, txtcontent):
        # txt=self.open_file()
        # print (str(txt))
        phone_type = re.findall('登陆成功!手机号:.*,请求头信息:(.*?)\n', txtcontent)
        # print (phone_type)
        return phone_type
    ...
    def Android_type_totle(self, IOS_and_Android):
        Android_type_totle = []
        Ios_type_totle = []
        print('android用户总数量:%s' % (len(IOS_and_Android[0])))
        for i in range(len(IOS_and_Android[0])):
            # print (IOS_and_Android[0][i - 1])
            android_juti_type = re.findall('Android(.*?)', IOS_and_Android[0][i - 1])  #下次如果用到修改这里，会有问题
    ...
    def Ios_type_totle(self, IOS_and_Android):
        Android_type_totle = []
        Ios_type_totle = []
        print('ios用户总数量:%s' % (len(IOS_and_Android[1])))
        for i in range(len(IOS_and_Android[1])):
            ios_juti_type = re.findall('IOS_(.*?)_', IOS_and_Android[1][i - 1])
